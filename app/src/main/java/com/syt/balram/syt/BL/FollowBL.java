...
public class FollowBL {
    ...
    public String addFollow(String email,String id)
    {
        String result=getDataStatus(email,id);
        String status=validate(result);
        return status;
    }

    private String getDataStatus(String email,String id)
    {

        String text = null;
        HttpClient httpClient = new DefaultHttpClient();
        HttpContext localContext = new BasicHttpContext();
        String url="email="+email+"&loginId="+id;

        try {
            URI uri = new URI("https", "www.sellyourtime.in", "/api/follow.php", url, null);
